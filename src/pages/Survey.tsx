...
export const Survey = () => {
 return (
    ...
    <div className="rounded-lg shadow-lg bg-white p-8 w-full max-w-md">
        <h1 className="text-2xl font-bold text-gray-800 mb-6 text-center">Encuesta</h1>
        ...
        {surveyData.map((options) => (
            <li key={options.id}>
            <h2 className="text-gray-700 font-semibold mb-2">{options.title}</h2>
            <div className="flex justify-between">
                {options.question.options.map((select) => (
                <label key={select.label} className="flex items-center gap-2">
                    <input type="radio" name={options.id} value={select.label} />
